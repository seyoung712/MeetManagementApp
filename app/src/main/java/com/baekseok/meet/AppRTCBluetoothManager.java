/* 용어 설명
SCO => Synchronous Connection-Oriented / Master-Slave 구조로 연결되는 음성채널임.
번역시 Reference Document를 참고하였으나, 의미 설명 변경이 불가능한 것은 로그수정 하지 않음.
*/
...
public class AppRTCBluetoothManager {
  ...
  public void updateDevice() {
    ...
    if (devices.isEmpty()) {
      bluetoothDevice = null;
      bluetoothState = State.HEADSET_UNAVAILABLE;
      Log.d(TAG, "블루투스 장치 연결안됨.");
    } else {
      // Always use first device in list. Android only supports one device.
      bluetoothDevice = devices.get(0);
      bluetoothState = State.HEADSET_AVAILABLE;
      Log.d(TAG, "블루투스 장치와 연결됨 : "
              + "name=" + bluetoothDevice.getName() + ", "
              + "state=" + stateToString(bluetoothHeadset.getConnectionState(bluetoothDevice))
              + ", SCO audio=" + bluetoothHeadset.isAudioConnected(bluetoothDevice));
    }
    Log.d(TAG, "블루투스 상태 업데이트 =" + bluetoothState);
  }
  ...
  protected void logBluetoothAdapterInfo(BluetoothAdapter localAdapter) {
    Log.d(TAG, "블루투스 어댑터 상태: "
            + "사용여부=" + localAdapter.isEnabled() + ", "
            + "상태=" + stateToString(localAdapter.getState()) + ", "
  ...
  private void bluetoothTimeout() {
    ...
    Log.d(TAG, "블루투스 연결 실패(시간초과): 블루투스 상태=" + bluetoothState + ", "
            + "attempts: " + scoConnectionAttempts + ", "
            + "SCO is on: " + isScoOn());
    if (bluetoothState != State.SCO_CONNECTING) {
      return;
